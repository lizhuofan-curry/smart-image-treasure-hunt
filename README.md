...
| 下游分类验证 | 同一个分类模型分别处理原图、加噪图和去噪图，页面并列展示类别与 Softmax 置信度，直观检查去噪后商品语义是否保持。 |
...
- **去噪效果可被下游任务验证**：点击图像去噪后，系统会用同一个分类模型分别处理原图、混合噪声图和 U-Net 去噪图，对比三者的预测类别与 Softmax 置信度，并判断去噪后的类别是否与原图一致。
...
| 图像去噪 | 带跳跃连接的轻量 U-Net 风格卷积自编码器，并复用分类模型进行下游验证 | 原图、混合噪声图、去噪图，以及三阶段类别与置信度对比 |
...
├── .github/workflows/ci.yml        # Python 语法与必需文件自动检查
...
- **持续集成**：每次推送至 `main` 或提交 Pull Request 时，GitHub Actions 会编译全部 Python 模块，并检查 README、依赖清单、标签表、Web 入口及模型资产是否存在。
- **下游一致性展示**：去噪路由复用已经常驻内存的分类服务，对原图、加噪图和去噪图依次推理，不需要重复加载模型或额外训练权重。
